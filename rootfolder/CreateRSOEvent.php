<?php
    require_once 'DBInfo.php';
    require_once 'functions.php';

    $inData = getRequestInfo();

    # Contact Book User registration information stored as variables.
    $event_ID = $inData["event_ID"];
    #$desc1 = $inData["desc1"];
    #$time =  $inData["time"];         #date('m-d-Y H:i');
    #$loc_name = $inData["loc_name"];
    #$event_name = $inData["event_name"];
    $approved = 0;
    $admin_ID = $inData["admin_ID"]; #which admin approved event
    #$pri_superadmin_ID = "1";
    
    #$Email = $inData["Email"];


    #$conn = $db;
    $conn = new mysqli($serverName, $dBUsername, $dBPassword, $dBName);
    if( $conn->connect_error )
    {
		returnWithError( $conn->connect_error );
    }

    # Query the database to insert the registered user into the Users table if
	# validation constraints are met or else return an error
    else
    { 
			$stmt = $conn->prepare("INSERT INTO rso_events ( event_ID, admin_ID) VALUES (?,?)");
            $stmt->bind_param("ss",  $event_ID,$admin_ID);
            $stmt->execute();
	        //$sid = $stmt->insert_id;   #check this one need to get UID
	        returnWithInfo($event_ID, $admin_ID );
            $stmt->close();
            $conn->close();
	
    }
    function returnWithInfo( $event_ID, $admin_ID )
	{
		$retValue = '{"RSO_ID": ' . $event_ID . ',"AdminID":' . $admin_ID . ',"error":""}';
		sendResultInfoAsJson( $retValue );
	}

?>